...
def check_user_type(user_input):
    if re.fullmatch(email_regex,user_input):
        ...
        data = {
            "success": False,
            "message": "Email, username yoki telefon raqamingiz notogri"
        }
        raise ValidationError(data)
    return user_input
...
def send_email(email, code):
    html_content = render_to_string(
        'email/authentication/activate_accaunt.html',
        {"code": code}
    )
    Email.send_email(
        {
...
def send_phone_code(phone,code):
    accaunt_sid = config('accaunt_sid')
    auth_token = config('auth_token')
    client = Client(accaunt_sid,auth_token)
    client.messages.create(
        body=f"Salom, sizning tastiqlash kodingiz {code} \n",
        from_="+99893311704",
        to=f"{phone}"
    )
